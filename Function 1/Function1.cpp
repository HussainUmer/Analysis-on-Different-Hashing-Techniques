...
public:
    ...
    int hash_function(const string& key, int attempt) {
        // Using two different hash functions for double hashing
        // Adjusting the table size to ensure they are relatively prime
        return (hash_function_1(key) + attempt * hash_function_2(key)) % size;
    }

    void insertLinearProbing(const string& key) {
        int index = hash_function(key, 0); // Initial index
        int attempt = 1;
    ...
    void insertDoubleHashing(const string& key) {
        ...
        while (!hash_table[index].empty()) {
            ...
            index = hash_function(key, attempt);
            attempt++;
        }
        hash_table[index] = key;
    }
...
int main() {
    ...
    while (start < paragraph.size()) {
        ...
        // table.insertLinearProbing(word); // Use linear probing
        // table.insertQuadraticProbing(word); // Use quadratic probing
        table.insertDoubleHashing(word); // Use double hashing
