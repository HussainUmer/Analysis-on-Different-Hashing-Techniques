...
int hashFunction(const string& s, int tableSize) {
    int index = 0;
    for (char c : s) {
        index = (index * 26 + (c - 'a' + 1)) % tableSize;
    }
    return index;
}
...
int secondaryHashFunction(const string& s, int tableSize) {
    int index = 0;
    for (char c : s) {
        index += c; // Accumulate ASCII values
    }
...
public:
    ...
    void insertDoubleHashing(const string& s) {
        ...
        while (!table[index].empty()) {
            ...
            // If we've traversed the whole table without finding an empty slot, return
